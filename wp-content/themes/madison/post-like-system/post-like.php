...
function like_scripts() {
    ...
    wp_enqueue_script('jm_like_post', get_template_directory_uri() . '/post-like-system/js/post-like.min.js', array('jquery'), '1.0', 1);
...
function show_user_likes($user) {
    ...
    <table class="form-table">
        <tr>
            <th><label for="user_likes"><?php _e('You Like:', AZEXO_THEME_NAME); ?></label></th>
            <td>
                <?php
...
function frontEndUserLikes() {
    if (is_user_logged_in()) { // user is logged in
        ...
        if ($count > 0) {
            ...
            $like_list .= "<aside>\n";
            $like_list .= "<h3>" . __('You Like:', AZEXO_THEME_NAME) . "</h3>\n";
            $like_list .= "<ul>\n";
...
function jm_most_popular_today() {
    ...
    if ($pop_posts->have_posts()) {
        echo "<aside>\n";
        echo "<h3>" . _e('Today\'s Most Popular Posts', AZEXO_THEME_NAME) . "</h3>\n";
        echo "<ul>\n";
...
function jm_most_popular_month() {
    ...
    if ($pop_posts->have_posts()) {
        echo "<aside>\n";
        echo "<h3>" . _e('This Month\'s Most Popular Posts', AZEXO_THEME_NAME) . "</h3>\n";
        echo "<ul>\n";
        while ($pop_posts->have_posts()) {
            $pop_posts->the_post();
            echo "<li><a href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular_week() {
    ...
    if ($pop_posts->have_posts()) {
        echo "<aside>\n";
        echo "<h3>" . _e('This Week\'s Most Popular Posts', AZEXO_THEME_NAME) . "</h3>\n";
        echo "<ul>\n";
        while ($pop_posts->have_posts()) {
            $pop_posts->the_post();
            echo "<li><a href='" . get_permalink($post->ID) . "'>" . get_the_title() . "</a></li>\n";
...
function jm_most_popular() {
    global $post;
    echo "<aside>\n";
    echo "<h3>" . _e('Most Popular Posts', AZEXO_THEME_NAME) . "</h3>\n";
    echo "<ul>\n";
    $args = array(
        'post_type' => array('post', 'enter-your-comma-separated-post-types-here'),
        'meta_query' => array(
